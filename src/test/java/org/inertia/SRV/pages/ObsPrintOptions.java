...
public class ObsPrintOptions {
    public static  WebDriver driver;
    public WebDriverWait wait;
    public static String firstLinkText;
    public static String secondLinkText;

    public ObsPrintOptions(WebDriver driver) {
        ObsPrintOptions.driver = driver;
        wait = new WebDriverWait(ObsPrintOptions.driver, 25);
    }

    public String clickOBSPrintButtonAndDownloadPrintPDFFile() throws InterruptedException, AWTException {
        try {
            Thread.sleep(4000);
            WebElement DocxLink = driver.findElement(By.xpath("(//div[contains(@class,'x-panel inertia-margin-top')])[2]//a[contains(text(),'.docx')][1]"));
             firstLinkText = DocxLink.getText();
            System.out.println("Text of the DOCX link: " + firstLinkText);
            Thread.sleep(3000);

            // Find the second link
            WebElement CSVLink = driver.findElement(By.xpath("(//div[contains(@class,'x-panel inertia-margin-top')])[2]//a[contains(text(),'.csv')][1]"));
             secondLinkText = CSVLink.getText();
            System.out.println("Text of the CSV link: " + secondLinkText);

            WebElement Printoption = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("(//span[text()='Print Options'])[last()]")));
            Printoption.click();
    ...
    public static String getFirstLinkText() {
        WebElement DocxLink1 = driver.findElement(By.xpath("(//div[contains(@class,'x-panel inertia-margin-top')])[2]//a[contains(text(),'.docx')][1]"));
